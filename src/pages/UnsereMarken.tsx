...
export default function UnsereMarken() {
  ...
  useEffect(() => {
    ...
    if (brandParam && brands) {
      const match = brands.find(b => b.name.toLowerCase() === brandParam.toLowerCase() || b.slug === brandParam.toLowerCase());
      if (match) setTimeout(() => document.getElementById(`brand-${match.slug}`)?.scrollIntoView({ behavior: "smooth" }), 300);
    }
  }, [searchParams, brands]);

  const scrollToLetter = (letter: string) => document.getElementById(`letter-${letter}`)?.scrollIntoView({ behavior: "smooth" });
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1">
        ...
        <div className="container mx-auto px-4 py-10">
          ...
          {error && <div className="text-center py-16 text-muted-foreground">{t("brands.loading_error")}</div>}
          {!isLoading && !error && brands && brands.length === 0 && (
            <div className="text-center py-16 text-muted-foreground">
              <p className="text-lg mb-2">{t("brands.empty")}</p>
              <p className="text-sm">{t("brands.empty_hint")}</p>
            </div>
          )}
          {!isLoading && !error && brands && brands.length > 0 &&
            ALPHABET.filter(l => grouped.has(l)).map(letter => (
              <div key={letter} id={`letter-${letter}`} className="mb-10 scroll-mt-32">
                <div className="flex items-center gap-3 mb-4">
                  <span className="flex items-center justify-center w-10 h-10 rounded-full bg-primary text-primary-foreground font-bold text-lg">{letter}</span>
                  <div className="flex-1 h-px bg-border" />
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {grouped.get(letter)!.map(brand => (
                    <LocaleLink key={brand.slug} id={`brand-${brand.slug}`} to={`/unsere-marken/${brand.slug}`}
