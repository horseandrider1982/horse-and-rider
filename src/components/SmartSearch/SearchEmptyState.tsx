        ...
        <div className="flex flex-wrap gap-2">
          {POPULAR_SEARCHES.map((s) => (
            <button
              ...
              className={cn(
                "px-4 py-2 rounded-full text-sm font-medium border border-border",
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
          {POPULAR_CATEGORIES.map((cat) => (
            <button
              key={cat.label}
              onClick={() => onChipClick(cat.query)}
              className={cn(
                "px-4 py-3 rounded-xl text-sm font-medium border border-border",
                "bg-muted/30 text-foreground hover:bg-primary hover:text-primary-foreground",
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {trending.map((p) => (
              <button
                key={p.handle}
                onClick={() => onProductClick(p.handle)}
                className="group text-left rounded-xl border border-border bg-background hover:shadow-md transition-all overflow-hidden"
              >
