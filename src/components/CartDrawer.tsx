...
export const CartDrawer = () => {
  ...
  const itemsDesc = totalItems === 0
    ...
    : totalItems === 1
      ? t("cart.items_one")
      : t("cart.items_other", { count: totalItems });
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button variant="outline" size="icon" className="relative">
          ...
          {totalItems > 0 && (
            <Badge className="absolute -top-2 -right-2 h-5 w-5 rounded-full p-0 flex items-center justify-center text-xs bg-primary text-primary-foreground">{totalItems}</Badge>
          )}
        </Button>
      </SheetTrigger>
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full">
        <SheetHeader className="flex-shrink-0">
          ...
          <SheetDescription>{itemsDesc}</SheetDescription>
        </SheetHeader>
                    ...
                    <div key={item.variantId} className="flex gap-4 p-2 border-b border-border">
                      <div className="w-16 h-16 bg-muted rounded overflow-hidden flex-shrink-0">
                        {item.product.node.images?.edges?.[0]?.node && (
                          <img src={item.product.node.images.edges[0].node.url} alt={item.product.node.title} width={64} height={64} className="w-full h-full object-cover" />
                        )}
                      </div>
